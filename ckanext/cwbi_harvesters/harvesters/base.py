...
from ckanext.cwbi_harvesters.validation import (MinimalFGDCValidator, FGDC1998Schema,
...
class CWBIHarvester(SpatialHarvester):

    def extra_schema(self):
        log.debug('Getting extra schema for CWBIHarvester')
        return {
    ...
    def get_package_dict(self, iso_values, harvest_object):
        ...
        package_dict = super(CWBIHarvester, self).get_package_dict(iso_values, harvest_object)
        if not package_dict:
            return None
...
class CWBICSWHarvester(CSWHarvester, CWBIHarvester):
    '''
    A Harvester for CSW servers, with customizations for CWBI
    '''
...
class CWBIDocHarvester(DocHarvester, CWBIHarvester):
    '''
    A Harvester for single spatial metadata docs, with customizations for CWBI
    '''
    def info(self):
        return {
            'name': 'single-doc',
...
class CWBIGeoportalHarvester(CSWHarvester, CWBIHarvester):
    '''
    A Harvester for CSW servers, with customizations for CWBI
    '''
    def info(self):
